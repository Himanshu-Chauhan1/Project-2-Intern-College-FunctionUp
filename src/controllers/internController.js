...
const createIntern = async function (req, res) {
    try {
        ...
        if (Object.keys(data).length == 0) {
            return res.status(400).send({ status: false, message: "enter details to create an intern" });
        }
        if (!data.name) {
            return res.status(400).send({ status: false, message: "candidate name is required" });
        }
        if (!/^[a-zA-Z ]*$/.test(data.name)) {
            return res.status(400).send({ status: false, massege: "plz enter valid name with title & dont use number or special characters", });
        }
        if (!data.email) {
            return res.status(400).send({ status: false, message: "candidate email id is required" });
        }
        ...
        if (!data.mobile) {
            return res.status(400).send({ status: false, message: "candidate mobile no is required" });
        }
        if (!/^(?:(?:\+|0{0,2})91(\s*|[\-])?|[0]?)?([6789]\d{2}([ -]?)\d{3}([ -]?)\d{4})$/.test(
            data.mobile
        )) {
            return res.status(400).send({ status: false, message: "plz enter a valid Mobile no" });
        }
...
const getCollegeDetails = async function (req, res) {
    try {
        ...
        if (!clgName) {
            return res.status(400).send({ status: false, message: "enter a college name first", });
        }
        let collegeName = req.query.collegeName.toUpperCase();
        const isValidCollege = await collegeModel.findOne({ name: collegeName }); //{ _Id}
        if (!isValidCollege) {
            return res.status(400).send({ status: false, message: "you have entered a invalid college name ", });
        }
        ...
        if (getIntern.length == 0) {
            return res.status(404).send({ status: false, message: "no intern found with your provided college details", });
        }
        const getAllIntern = {
